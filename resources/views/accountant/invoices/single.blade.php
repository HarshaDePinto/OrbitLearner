          ...
          <div class="row">
            <div class="col-lg-12">
              <div class="invoice-title">
                <h2>Invoice</h2>
                <div class="invoice-number">{{$invoice->user->title}} {{$invoice->user->f_name}} {{$invoice->user->l_name}}</div>
              </div>
              ...
              <div class="table-responsive">
                <table class="table table-striped table-hover table-md">
                  ...
                  @foreach ($invoice->items as $index=>$item)
                          ...
                          <tr>
                              <td>{{$index+1}}</td>
                              <td>{{$item->b_payment->g_batch->group->grade->title}} |{{$item->b_payment->g_batch->group->title}} | {{$item->b_payment->g_batch->title}} | {{$item->b_payment->g_batch->year}}
                                <div class=" text-small font-600-bold">
                                    <span class="text-info">{{$item->b_payment->g_batch->user->title}} {{$item->b_payment->g_batch->user->f_name}} {{$item->b_payment->g_batch->user->l_name}}</span>
